...
for episode in range(config.num_generations):
    ...
    for policy in range(config.num_policies):#test each member of the population 
    ...
    if (episode % config.checkpoint_freq == 0) and  (episode != 0):
        ...
        with open(config.model_path + str(episode) + 'Champion_Network.pk1', 'wb') as output:#saves the entire network
        ...
        for i in range(config.episodes_to_solve):
            summed_reward += champ.playthrough(env)
